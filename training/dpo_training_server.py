# training\dpo_training_server.py
...
try:
    ...
    from dpo_training import (  # type: ignore
        _build_manual_configuration,
        run_manual_training,
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Server-oriented entry point for DPO fine-tuning.")
    parser.add_argument(
        "--model",
        type=str,
    ...
    log_path = configure_output_streams("dpo_server", args.model, args.log_tag)
    sys.__stdout__.write(f"[dpo_server] Logging redirected to {log_path}\n")
    ...
    print("==" * 60)
    print(f"[DPO Server Config] Model: {model_cfg}\n")
    print(f"[DPO Server Config] Training: {training_cfg}")
    print("==" * 60 + "\n")

    if args.dry_run:
        print("[dpo_server] Dry run requested; skipping training execution.")
        return

    print("[dpo_server] Starting DPO fine-tuning run...")
    run_manual_training(model_cfg, training_cfg)
    print("[dpo_server] Training run completed.")
