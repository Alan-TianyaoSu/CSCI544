"""
Log-likelihood winner selection for SFT or RLHF evaluation sets with artifact logging.

Usage:
    python evaluate/sft_metrics.py --model gemma-3-1b-it
"""
...
APPEND_EOS = True
RESULTS_ROOT = Path("results") / "preference_selection"
...
def create_result_dir(task: str, model_name: str) -> Tuple[Path, str]:
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    task_component = sanitize_for_path(task)
    model_component = sanitize_for_path(model_name)
    result_dir = RESULTS_ROOT / task_component / model_component / timestamp
...
def plot_margin_distribution(sample_rows: List[dict], output_path: Path, weight_key: Optional[str] = None) -> None:
    ...
    ax.set_title("Score margin distribution (response A - response B)")
...
def evaluate_model(
    ...
    for sample_idx, (prompt, resp_a, resp_b, label) in enumerate(
        ...
        try:
            score_a, trunc_a = compute_response_score(
                model,
                tokenizer,
                prompt_ids,
                resp_a,
            ...
            score_b, trunc_b = compute_response_score(
                model,
                tokenizer,
                prompt_ids,
                resp_b,
        ...
        truncated_pairs += int(trunc_a or trunc_b)
        ...
        sample_records.append(
            {
                ...
                "label": label,
                "prediction": pred,
                "score_a": safe_float(score_a),
                "score_b": safe_float(score_b),
                "margin": safe_float(margin_value) if margin_value is not None else None,
                "is_correct": bool(pred == label),
                "truncated_a": bool(trunc_a),
                "truncated_b": bool(trunc_b),
            }
        )
    ...
    if margins:
        margins_array = np.array(margins, dtype=np.float32)
        metrics.update(
            {
                "margin_std": float(np.std(margins_array)),
                "margin_min": float(np.min(margins_array)),
                "margin_max": float(np.max(margins_array)),
                "margin_median": float(np.median(margins_array)),
                "positive_margin_ratio": float(np.mean(margins_array > 0)),
                "positive_margin_count": int(np.sum(margins_array > 0)),
            }
        )
    else:
        metrics.update(
...
def main() -> None:
    ...
    run_dir, timestamp = create_result_dir(args.task, args.model)
    run_info = {
        "timestamp": timestamp,
        "task": args.task,
        "model": args.model,
    ...
    print(f"[Artifacts] Results will be stored under: {run_dir}")
    ...
    save_evaluation_artifacts(run_dir, "base_model", base_metrics, base_samples)
    ...
    print(f"\n[Summary] Artifact directory: {run_dir}")
